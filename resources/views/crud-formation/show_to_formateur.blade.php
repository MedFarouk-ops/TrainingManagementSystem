    ...
    <div class="row">
        <div class="col-md-5">
            ...
            <div class="project-info-box">
                ...
                <p><b>Date debut: </b> {{$formation->date_debut}}</p>
                <p><b>Date fin: </b> {{$formation->date_fin}}</p>
                <p><b>Date debut: </b> {{$formation->date_debut}}</p>
                <p><b>Duree: </b>  {{$formation->duree}}</p>
               
            </div><!-- / project-info-box -->

            <div class="project-info-box mt-0 mb-0">
                <p class="mb-0">
                   <button type="button" class="btn btn-success" disabled>Inscrire</button>
                   @if (Auth::guard('formateur')->user()->id == $formation->formateur_id)
                      <a href="{{ route('formations.edit' ,$formation->id) }}" type="button" class="btn btn-info" >Modifier</a>
                       
                  @else
                     <button type="button" class="btn btn-info" disabled>Modifier</button>
                  @endif
                  <a href="{{ route('formateur.dashboard') }}" type="button" class="btn btn-warning" >Retour</a>
        ...
        <div class="col-md-7">
            ...
            <div class="project-info-box">
                <p><b>Certification : </b>{{$formation->certified}}</p>
                 <p><b>Categorie: </b>  {{$formation->categorie}}</p>
                 <form action="{{ route('formations.destroy', $formation->id)}}" method="post">
                            @csrf
                            @method('DELETE')
                          <input type="submit" name="delete" value="Supprimer" class="btn btn-danger my-2 " />
                  </form>
            </div><!-- / project-info-box -->
        </div><!-- / column -->
    </div>

<!doctype html>
                        <html>
                            <head>
                                ...
                                <title>Gestion De l'inscription au plateform</title>
...
.add-experience:hover {
    ...
    <div class="row">
        ...
        <div class="col-md-3 border-right">
            ...
            <form action="{{route('permission.edit.submit',$permission->id)}}" method="post">
                @csrf
                @method('PUT')
                
                <input type="radio" name="status"  <?php echo ($permission->status=='accepted')?'checked':'' ?> value="accepted">
                <label>Acceptée </label>
                <br>
                <input type="radio" name="status" <?php echo ($permission->status=='awaiting')?'checked':'' ?>  value="awaiting">
                <label>En attend</label>
                <br>
                <input type="radio" name="status"  <?php echo ($permission->status=='rejected')?'checked':'' ?>  value="rejected">
                <label>Rejetée</label>
